...
def mainCalculation():
    ...
    cF = lambda u: varPhi(u) * np.exp(i*u* (np.log(S0)+ r+ omega -0.5*sigma*sigma)*t - 0.5*sigma*sigma *u *u *t)  

    # Cumulants needed for the integration range

    zeta1 = (np.log(S0)+ r + omega -0.5*sigma*sigma)*t + t*C*Y*stFunc.gamma(-Y)*(np.power(G,Y-1)-np.power(M,Y-1))
